import React, { useEffect } from 'react';
import { useLoaderData, useParams } from 'react-router-dom';

const SingleCollage = () => {

const singleCollageData = useLoaderData()
console.log(singleCollageData );

const

{
 _id,
 college_image,
 college_name,
 events,
 admission_dates,
 sports ,
 research_history
} = singleCollageData;

    return (
        <div>
            <div className=" card-side bg-black text-white shadow-xl  font-serif ">
        <figure><img src={college_image} className='rounded-md h-full' alt="Album"/></figure>
        <div className="card-body">
          <h2 className="card-title text-3xl">{college_name}</h2>
          <div  className='mt-1' >
            Admission Date: 
          <span className='ms-2'>
          {admission_dates.fall}
          </span>
            </div>

           <div  className='mt-2' >Research History :
            <span className='ms-2'>{research_history}</span>
            </div> 

            <div>
                <p> UpComing Events</p>

                <ul  className='mt-2'>
                   {events.map((event, index) => 
                   <li> {index +1} .{event}</li> 
                   )}

                </ul>
            </div>

            <div>
             <p>Sports</p>

             <ul className='mt-2'>
                {sports.map(
                    (sport, index)=>
                    
                    <li>{index + 1}. {sport}</li>
                )}
             </ul>
            </div>


          

        </div>
      </div>
        </div>
    );
};

export default SingleCollage;